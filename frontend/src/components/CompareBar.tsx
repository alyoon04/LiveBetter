...
export function CompareBar({ selectedMetros, onRemove, onClear }: CompareBarProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#0C0C14] border-t border-[#1E1E2A] shadow-2xl z-50">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1 overflow-x-auto">
            ...
            <div className="whitespace-nowrap">
              ...
              <span className="ml-2 text-xs text-[#6B6B7E]">
            ...
            <div className="flex gap-2">
              {selectedMetros.map((metro) => (
                <div
                  key={metro.metro_id}
                  className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#111118] border border-[#1E1E2A] rounded text-sm"
                >
                  <div>
                    <span className="font-medium text-white text-xs">{metro.name}, {metro.state}</span>
                    <span className="ml-2 font-mono text-[10px] text-[#6B6B7E]">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onClear}
              className="text-xs text-[#6B6B7E] hover:text-white transition-colors whitespace-nowrap"
            >
              Clear
            </button>
            {selectedMetros.length >= 2 && (
              <Link
                ...
                className="px-5 py-2 bg-primary-400 hover:bg-primary-300 text-[#0C0C14] rounded font-display font-bold text-sm transition-colors whitespace-nowrap"
              >
